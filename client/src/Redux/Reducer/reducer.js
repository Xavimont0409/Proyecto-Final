...
const Reducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case GET_ALL_VACANTS:
      const filteredVacants = action.payload
        .filter((vacant) => {
          return ["Hiper Destacado", "Destacado", "Basico", "none"].includes(
            vacant.operation
          );
        })
        .sort((a, b) => {
          const order = ["Hiper Destacado", "Destacado", "Basico", "none"];
          return order.indexOf(a.operation) - order.indexOf(b.operation);
        });

      return {
        ...state,
        Vacant: filteredVacants,
        AuxVacant: filteredVacants,
        AuxVacant2: filteredVacants,
      };
    ...
    case FILTER_PER_SENIORITY:
      ...
      const filterPerSeniority =
        action.payload === "senior"
          ? seniorityVacant.filter((vacant) =>
              vacant.Seniority.name.includes(action.payload)
            )
      ...
      const filterPerWordkmethod =
        action.payload === "presencial"
          ? wordkmethodVacant.filter((vacant) =>
              vacant.WorkMethod.name.includes(action.payload)
            )
    ...
    case FILTER_PER_TIME:
      ...
      const filterPerTime =
        action.payload === "Hoy"
          ? timeVacant.filter(
              (vacant) =>
                vacant.createdAt.slice(0, 10) === `${anios}-${month}-${dia}`
            )
          : action.payload === "Esta semana"
          ? timeVacant.filter(
              (vacant) =>
                vacant.createdAt.slice(0, 10) > `${anios}-${month}-${dia - 8}`
            )
          : action.payload === "Este mes"
          ? timeVacant.filter(
              (vacant) =>
                vacant.createdAt.slice(0, 10) > `${anios}-${month - 1}-${dia}`
            )
          : state.AuxVacant2;
      return {
        ...state,
